...
boot_alloc(uint32_t n)
{
	...
	{
		nextfree = ROUNDUP((char *)nextfree + (pages_required * PGSIZE), PGSIZE);
		return (uint64_t *) next_free_page;
	}
	/*new code*/
...
x64_vm_init(void)
{
	...
	size_t j;
	j = ROUNDUP(npages * sizeof(struct PageInfo), PGSIZE );
	boot_map_region(pml4e, UPAGES, j, PADDR(pages), PTE_P | PTE_U);

	j = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
	boot_map_region(pml4e, UENVS, j, PADDR(envs),  PTE_P | PTE_U);
	//////////////////////////////////////////////////////////////////////
...
page_init(void)
{
	...
	physaddr_t next_free_phy_mem;
	next_free_phy_mem = PADDR(boot_alloc(0));

	for (i = 0; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = NULL;

		if( (i == PGNUM(MPENTRY_PADDR)) || ((i>=(PGNUM(IOPHYSMEM)) && i<=(PGNUM(next_free_phy_mem)))||(i==0)))
		{
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
			continue;
...
//	return 0;
	...
	if(alloc_flags && ALLOC_ZERO)
		memset((void *)page2kva(newpage),'\0',PGSIZE);
	else
		memset((void *)page2kva(newpage), 0, PGSIZE);
...
pml4e_walk(pml4e_t *pml4e, const void *va, int create)
{
	...
	if(!(phy & PTE_P))
	{
		...
		{ 
			...
			pp->pp_ref++;
			pdpe = page2kva(pp);
			pml4e[PML4(va)] = PADDR(pdpe)|PTE_U|PTE_P|PTE_W;
	...
	{
		pdpe = (pdpe_t *)KADDR(PTE_ADDR(phy));
		pte = pdpe_walk(pdpe, va, create);
		return pte;
	}
}
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	...
	if(!(phy & PTE_P))
	{
		...
		else
		{
			...
			pp->pp_ref++;
			pgdir = page2kva(pp);
			pdpe[PDPE(va)] = PADDR(pgdir)|PTE_U|PTE_P|PTE_W;
	...
	{ 	
		pgdir = (pde_t *)KADDR(PTE_ADDR(phy));
		pte = pgdir_walk(pgdir, va, create);
		return pte;
	}
}
...
{
	// Fill this function inin

	pte_t *pte;
	struct PageInfo *pp;

	physaddr_t phy = pgdir[PDX(va)];	
	...
	if(!(phy & PTE_P))
	{
		...
		else
		{
			...
			pp->pp_ref++;
			pte = page2kva(pp);
			pgdir[PDX(va)] =PADDR(pte)|PTE_U|PTE_P|PTE_W;
			pte= &pte[PTX(va)];
			return pte;
		}
	}else
	{	
		pte = (pte_t *)KADDR(PTE_ADDR(phy));
		pte= &pte[PTX(va)];	
		return pte;
	}
}
...
boot_map_region(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	{
		p = pml4e_walk(pml4e, (const void *)( la + i), 1); 
		*p = PTE_ADDR(pa + i) | perm | PTE_P;
	}	  
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	uintptr_t addr;
        ...
        if( (size+base) > MMIOLIM)
                panic("Not enough memory for %s",__func__);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	start = (uintptr_t) ROUNDDOWN((uintptr_t)va, PGSIZE);
	end = (uintptr_t) ROUNDUP((uintptr_t)va+len, PGSIZE);	
        for( ; start < end; start = start + PGSIZE)
        {
                pte = (pte_t *)pml4e_walk(env->env_pml4e ,(const void *)start, 0);
                if( (!pte) || (!(*pte & perm)))
                {
                        user_mem_check_addr = (uintptr_t)va>start?(uintptr_t)va:(uintptr_t)start;
                        return -E_FAULT;
                }
        }
	return 0;
