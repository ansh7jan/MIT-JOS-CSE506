	...
	for(start = UTEXT; start < UXSTACKTOP-PGSIZE; start = start+PGSIZE)
         {
               	if(!(uvpml4e[VPML4E(start)] & PTE_P))
                        continue;
                if(!(uvpde[VPDPE(start)] & PTE_P))
                        continue;
                if(!(uvpd[VPD(start)] & PTE_P))
                        continue;
		
		if((uvpt[VPN(start)] & PTE_SHARE)==PTE_SHARE)
                {
			r = sys_page_map(0, (void *)start, child, (void *)start, uvpt[VPN(start)] & PTE_SYSCALL);
			if (r<0)
				return r;
		}
	}
