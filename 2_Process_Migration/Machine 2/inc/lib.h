...
int sys_transmit_packet(char *packet, uint32_t len);
int sys_receive_packet(char *packet, int *len);
int setstatus(int psid, int status);
int sys_pageinfo(envid_t env_id, uint32_t *addr, void *temp_mem, int *status);
int sys_settrapframe(envid_t *env_id, struct Trapframe *tf);
int sys_pageinsert(envid_t env_id, void *temp_mem, uint8_t *addr);
