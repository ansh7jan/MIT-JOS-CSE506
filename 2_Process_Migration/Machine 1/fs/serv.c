...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if (r > PGSIZE)
		r = PGSIZE;	
	r = file_read((struct File *)of->o_file, (void *)ret->ret_buf, r, of->o_fd->fd_offset);
	if (r<0)
		return r;
	of->o_fd->fd_offset += r;
	return r;
...
// Simillar to the read function 
// Point 5 Challenge problem
int
serve_write(envid_t envid, struct Fsreq_write *req)
{
	int r;
	struct OpenFile *of;
	struct OpenFile **temp = &of;
        r = openfile_lookup(envid,req->req_fileid,&of);
	if (r<0)
		return r;
        r = file_write((*temp)->o_file,req->req_buf,req->req_n,(*temp)->o_fd->fd_offset);
	if (r<0)
		return r;
	(*temp)->o_fd->fd_offset = of->o_fd->fd_offset + r;
	return r;
}
